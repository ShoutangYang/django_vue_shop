...
class SmsSerializer(serializers.Serializer):
    ...
    def validate_mobile(self, mobile):
        ...
        one_minte_ago = datetime.now() - timedelta(hours=0,minutes=1,seconds=0)
        if VerifyCode.objects.filter(add_time__gt=one_minte_ago,mobile=mobile).count():
            raise serializers.ValidationError('距离上次验证不到60s')
...
class UserRegSerializer(serializers.ModelSerializer):
    ...
    def validate_code(self, code):
        ...
        if verify_records:
            ...
            five_minte_ago = datetime.now() - timedelta(hours=0, minutes=5, seconds=0)
            if  five_minte_ago>last_record.add_time:
